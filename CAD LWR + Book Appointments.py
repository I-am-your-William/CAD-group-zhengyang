...
db = sqlite3.connect('CAD_Database.db')
cursor = db.cursor()
...
canvas1.create_text(460, 125, text = "Pending Requests/Checkups:", font = ("Roboto", 14, "bold"), fill = "#004C7D")
listcontainer=tk.Frame(canvas1, bg="#FFFFFF", width=666, height=555)
listcontainer.pack_propagate(False)
canvas1.create_window(650, 420, window = listcontainer)
checkuplist=tk.Canvas(listcontainer, bg="#FFFFFF", width=666, height=555, highlightthickness=0)
checkuplist.pack(side="left", fill="both", expand=True)
checkuplist.pack_propagate(False)
scrollbar = tk.Scrollbar(listcontainer, orient=tk.VERTICAL, command=checkuplist.yview)
scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
checkuplist.config(yscrollcommand=scrollbar.set, scrollregion=(0,0,0,1000))
...
for i, item_text in enumerate(itemlist):
    frame = tk.Frame(checkuplist,height=150, bd=1, relief=tk.SOLID)
    frame.pack_propagate(False)
    frame.grid(row=i, column=0, sticky="ew")  # Set sticky to "ew" for horizontal expansion

    label = tk.Label(frame, text=item_text, padx=10, pady=10)
    label.pack(fill=tk.BOTH, expand=True)

# Configure column to expand horizontally
checkuplist.columnconfigure(0, weight=1)
...
def format_text(text, max_line_length=45):
    words = text.split()
    lines = []
    current_line = ""
    for word in words:
...
def view_clinic_info(event):
    ...
    def request_appointment():
        if not reqappdatebox.get() or not reqapptimebox.get() or not reqappsickbox.get():
        ...
        if result is not None:
            ...
            cursor.execute("INSERT INTO Appointment (clinic_id, patID, appdate, apptime, condition, appstatus) VALUES (?, ?, ?, ?, ?, ?)",
                                (clinic_id, patID, reqappdatebox.get(), reqapptimebox.get(), reqappsickbox.get(), 0))
            db.commit()
    ...
    infocv.create_text(200, 780, text = "Request for Appointment:", font = ("Roboto", 20, "bold"), fill = "#004C7D")
    reqappframe = Frame(infocv, bg="#FFFFFF", width=600, height=600)
    infocv.create_window(333, 1100, window = reqappframe)
    reqappdate = Label(reqappframe, text = "Date:", font = ("Roboto", 16), bg = "#FFFFFF")
    reqappdate.place(x=50, y=30)
    reqappdatebox = Entry(reqappframe, width=30, font = ("Roboto", 16), bd=2, bg="#B5DDF0")
    reqappdatebox.place(x=120, y=30)
    reqapptime = Label(reqappframe, text = "Time:", font = ("Roboto", 16), bg = "#FFFFFF")
    reqapptime.place(x=50, y=100)
    reqapptimebox = Entry(reqappframe, width=30, font = ("Roboto", 16), bd=2, bg="#B5DDF0")
    reqapptimebox.place(x=120, y=100)
    reqappsick = Label(reqappframe, text = "Describe Your Condition:", font = ("Roboto", 16), bg = "#FFFFFF")
    reqappsick.place(x=50, y=170)
    reqappsickbox = Entry(reqappframe, width=40, font = ("Roboto", 16), bd=2, bg="#B5DDF0")
    reqappsickbox.place(x=50, y=200)
    reqappbtn = Button(reqappframe, text = "Request Appointment", font = ("Roboto", 16), bg = "#054C76", fg = "White", width = 18, command=lambda: request_appointment())
    reqappbtn.place(x=180, y=350)
...
fetch_clinics()
clclist.bind("<Double-Button-1>", view_clinic_info)
